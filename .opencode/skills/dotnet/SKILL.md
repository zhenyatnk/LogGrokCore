---
name: dotnet
description: Use when building, testing, running, formatting, or verifying the C#/.NET solution (LogGrokX.sln), or when editing .cs, .xaml, .csproj, appsettings.yaml, or nlog.config. Covers the dotnet CLI workflow and this repo's warnings-as-errors rules.
---

# .NET workflow (LogGrokX)
...
For a faster inner loop, `dotnet build LogGrokX.sln` and
`dotnet test LogGrokX.sln` (incremental) are fine; `-t:Rebuild` before the
final check.
...
diagnostic logs to `%LOCALAPPDATA%\LogGrokX\`. If it exits immediately with
